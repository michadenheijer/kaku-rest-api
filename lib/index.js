...
program
    .option('-d, --device [path]', 'set a devicefile', function(p) { Plugin.addPluginPath(p); })
    .parse(process.argv);

if (program.device) {
    console.log("devicefile at: " + process.device);
    const deviceData = require(process.device);
} else {
    console.error("You have not defined a devicefile, it might work, but not everything will work correctly");
}

if (fs.existsSync(__dirname + "/" + "status.json")) {
